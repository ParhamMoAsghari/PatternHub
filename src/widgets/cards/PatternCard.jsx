...
function PatternCard({title, description, imageSRC, imageBase64 = null, link, placeholderSrc = "", className}) {
    return (
        <Link to={link} key={title}
            ...
            <ImagePlaceHolder key={title} placeholderSrc={placeholderSrc}>
                <img src={imageBase64 !== null ? `data:image/jpeg;base64,${imageBase64}` : imageSRC}
                     className="rounded pb-2" style={{width: 280 + 'px', height: 200 + 'px',}}
                     alt={title}/>
            </ImagePlaceHolder>
